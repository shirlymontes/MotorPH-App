...
/**
 * SideMenu class represents a custom JPanel with buttons for navigating 
 * between different forms such as EmployeeForm, TimesheetForm, and PayrollForm.
 */
public class SideMenu extends JPanel {

    public SideMenu() {
        initComponents();
        setBackground(new Color(6, 17, 60));  // Set background color
        setVisible(true);
        setOpaque(false);
    ...
    /**
     * Method to handle action events for the buttons.
     * @param e the ActionEvent triggered by button clicks
     */
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == ButtonEmployees) {
            loadForm(new EmployeeForm()); // Load EmployeeForm when ButtonEmployees is clicked
        } else if (e.getSource() == ButtonTimesheet) {
            loadForm(new TimesheetForm());  // Load TimesheetForm when ButtonTimesheet is clicked
        } else if (e.getSource() == ButtonPayroll) {
            loadForm(new PayrollForm()); // Load PayrollForm when ButtonPayroll is clicked
        }
    }
    ...
    private void loadForm(JPanel panel) {
        ...
        revalidate();
        repaint(); // Repaint the SideMenu to reflect the changes
    }
    ...
    private void ButtonEmployeesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ButtonEmployeesActionPerformed
       // Change the icon to the selected state for ButtonEmployees
    ...
    private void ButtonTimesheetActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ButtonTimesheetActionPerformed
       // Change the icon to the selected state for ButtonTimesheet
    ...
    private void ButtonPayrollActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ButtonPayrollActionPerformed
        // Change the icon to the selected state for ButtonPayroll       
        ButtonEmployees.setIcon(new ImageIcon(getClass().getResource("/com/group4/images/Employee.png")));
        ButtonTimesheet.setIcon(new ImageIcon(getClass().getResource("/com/group4/images/Timesheet.png")));
